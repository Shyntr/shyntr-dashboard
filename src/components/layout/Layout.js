...
function NavItem({item, mobile, onClose}) {
    ...
    const collapsibleContentId = `nav-section-${item.name.toLowerCase().replace(/\s+/g, '-')}`;
        ...
        return (
            <Collapsible open={open} onOpenChange={setOpen}>
                <CollapsibleTrigger asChild>
                    <button
                        ...
                        aria-label={`${item.name} navigation section`}
                        title={`${open ? 'Collapse' : 'Expand'} ${item.name}`}
                    >
            <span className="flex items-center gap-3">
              <item.icon className="h-5 w-5" strokeWidth={1.5}/>
                ...
                <CollapsibleContent id={collapsibleContentId} className="pl-8 space-y-1 mt-1">
...
export function Layout({children}) {
    ...
    return (
        <div className="relative min-h-screen bg-background">
            ...
            <div
                className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-violet-900/10 via-background to-background pointer-events-none"/>

            {/* Desktop Sidebar */}
            <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-64 lg:flex-col">
                <div
                    className="flex grow flex-col overflow-y-auto border-r border-border/40 bg-card/50 backdrop-blur-xl">
                    <Sidebar/>
                </div>
            </div>
            ...
            {/* Mobile Header */}
            <div
                className="sticky top-0 z-40 flex h-16 items-center gap-4 border-b border-border/40 bg-background/80 backdrop-blur-xl px-4 lg:hidden">
            ...
            {/* Desktop Header */}
            <div
                className="hidden lg:fixed lg:top-0 lg:right-0 lg:left-64 lg:z-40 lg:flex lg:h-16 lg:items-center lg:justify-end lg:gap-4 lg:border-b lg:border-border/40 lg:bg-background/80 lg:backdrop-blur-xl lg:px-6">
                <Button
                    variant="ghost"
                    size="icon"
                    onClick={toggleTheme}
